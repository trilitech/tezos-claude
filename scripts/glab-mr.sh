...
#
# Usage:
#   .claude/scripts/glab-mr.sh create <body-file> [--title <title>] [--base <base>] [--repo <repo>]
#   .claude/scripts/glab-mr.sh update <mr-id> <body-file> [--repo <repo>]
#
# Defaults:
#   <base>  = master
#   <repo>  = tezos/tezos
...
usage() {
    cat >&2 <<EOF
usage: $(basename "$0") create <body-file> [--title <title>] [--base <base>] [--repo <repo>]
       $(basename "$0") update <mr-id> <body-file> [--repo <repo>]
EOF
    exit 2
}
